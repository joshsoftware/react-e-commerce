...
const cartReducer = (state = initialState, action) => {
  switch (action.type) {
    case CART_REDUCER.SET_CART_ITEMS: {
      let newTotalPrice = 0, newTotalDiscount = 0, newNetTax = 0;
      action.value.map((product) => {
        newTotalPrice += product.product_price * product.quantity;
        newTotalDiscount += product.discount * product.quantity
        newNetTax += product.tax * product.quantity;
      });
      return { ...state, cartItemsList: action.value, totalPrice: newTotalPrice, totalDiscount: newTotalDiscount, tax: newNetTax };
    }
    ...
    case CART_REDUCER.DELETE_CART_ITEM: {
      ...
      let newTotalPrice = state.totalPrice, newTotalDiscount = state.totalDiscount, newNetTax = state.netTax;
      newTotalPrice -= newCartItemsList[index].product_price * newCartItemsList[index].quantity;
      newTotalDiscount -= newCartItemsList[index].discount * newCartItemsList[index].quantity;
      newNetTax -= newCartItemsList[index].tax * newCartItemsList[index].quantity;
      newCartItemsList.splice(index, 1);
      return { ...state, cartItemsList: newCartItemsList, totalPrice: newTotalPrice, totalDiscount: newTotalDiscount, tax: newNetTax };
    }
    case CART_REDUCER.UPDATE_ITEM_QUANTITY: {
      ...
      return { ...state, cartItemsList: newCartItemsList, totalPrice: newTotalPrice, newTotalPrice, totalDiscount: newTotalDiscount, tax: newNetTax };
    }
