...
const FormLabel = ({ field, labelText, mainLabel, setLabel, setProducts, products }) => {
  ...
  const filterFunction = (labelText) => {
    ...
    if (accessories.includes(labelText)) {
      ...
      if (!checked) {
        let products_arr = [];
        productList
          .filter((item) => item[`${mainLabel}`] === labelText)
          .map((filteredItem) => products_arr.push(filteredItem));
  ...
  const addFilters = (props) => {
    ...
    let color_arr = [];
    let brand_arr = [];
    arr.map((filteredProduct) => size_arr.push(filteredProduct.size));
    arr.map((filteredProduct) => color_arr.push(filteredProduct.color));
    arr.map((filteredProduct) => brand_arr.push(filteredProduct.brand));
