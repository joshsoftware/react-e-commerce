...
const Footer = () => {
  ...
  const partner = FooterData[1].partners;
    ...
    partner.map((partner) => {
        return arr1.push(
            <LiComponent key={partner.id} data={partner.name} />
        );
    });
  ...
  const contacts = FooterData[2].contactus;
    ...
    contacts.map((contact) => {
        return arr2.push(
            <LiComponent key={contact.id} data={contact.name} />
        );
    });
    ...
    a1.push(
        <ColumnWrapper key={'column-partners'} data={<UlComponent liElement={arr1} />} xs={4} />
    );
    a1.push(
